...
def create_package(
    ...
    prompt = {
        ...
        "requirements": "Return JSON with hook, narration, title, description, tags, and format_name. Use a format appropriate to the source from news_breakdown, fact_explainer, myth_bust, technical_joke, surprising_fact, timeline, question_answer, prediction_watch, reddit_story. For non-Reddit formats, begin narration with the exact source title, then give the source-backed development, implication, and qualified takeaway. Only use timeline or prediction_watch when the source supports that promise; only use reddit_story when the Reddit source explicitly identifies the author, community, and reuse permission. For reddit_story, read the exact source title first, then narrate the body through its real outcome or lesson; do not fabricate a satisfying ending that is absent from the post. Prefer a complete 35-55 second treatment when the source supports it. Add substantial original commentary and never present a Reddit user's account as independently verified fact. Be accurate, original, entertaining, educational, and do not give financial advice, investment recommendations, or cyber instructions.",
    }
    try:
        response = httpx.post(
